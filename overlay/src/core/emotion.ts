...
const NEEDS: Array<{ key: keyof PetStatus; bubble: string }> = [
  { key: 'fullness', bubble: 'еда?' },
  { key: 'cleanliness', bubble: 'мыло?' },
  { key: 'energy', bubble: 'сон?' },
  { key: 'happiness', bubble: 'мяч?' },
]
...
export function emotionFor(p: Presence): Emotion {
  ...
  if (p.kind === 'no-pet')
    return { frames: FALLBACK_FRAMES, bubble: 'заведи меня!', tint: 'warn', asleep: false }

  const s = p.status
  const frames = s.frames?.length ? s.frames : [s.display ?? '(?)']
